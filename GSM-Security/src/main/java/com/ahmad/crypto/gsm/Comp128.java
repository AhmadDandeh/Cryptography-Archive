...
public class Comp128 {
    ...
    public Comp128(String simKey) {
        ...
        String randomChallenge = generateRandom128Bit();

        // Initial state: Random challenge in the last 16 bytes
        for (int i = 0; i < 16; i++) {
            x[i + 16] = Integer.parseInt(randomChallenge.substring(i * 8, (i + 1) * 8), 2);
        }

        // 8 Iterations of the core transformation
        for (int i = 0; i < 8; i++) {
            // Load key into the first 16 bytes
            for (int j = 0; j < 16; j++) {
                x[j] = Integer.parseInt(simKey.substring(j * 8, (j + 1) * 8), 2);
            }

            // Butterfly-like substitution/permutation rounds
            for (int j = 0; j < 5; j++) {
                int step = 1 << (4 - j);
                int groupSize = 1 << (5 - j);
                for (int k = 0; k < (1 << j); k++) {
            ...
            // Permutation for the next iteration (except the last one)
            if (i < 7) {
                for (int j = 0; j < 16; j++) {
                    x[j + 16] = 0;
                    for (int k = 0; k < 8; k++) {
                        int nextBitIndex = (8 * j + k) * 17 % 128;
                        x[j + 16] |= (bit[nextBitIndex] << (7 - k));
                    }
                }
            }
        }

        // Final SRES (32 bits) and KeyC (54 bits + padding)
        StringBuilder sresBuilder = new StringBuilder();
        for (int j = 0; j < 32; j++) sresBuilder.append(bit[j]);
        this.SRES = sresBuilder.toString();

        StringBuilder keyBuilder = new StringBuilder();
        for (int j = 74; j < 128; j++) keyBuilder.append(bit[j]);
        this.keyC = keyBuilder.toString() + "0000000000";
    }

    private String generateRandom128Bit() {
        Random r = new Random();
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 128; i++) sb.append(r.nextInt(2));
        return sb.toString();
    }

    public String getKeyC() { return this.keyC; }
    public String getSRES() { return this.SRES; }
}